...
def main(spark, model_file, test_file, tag_file, index_file):
    ...
    table1 = spark.sql('select rec_table.value as track_id, ifnull(count(rec_table.value),0) as num_rec, ifnull(listen_table.num_listen,0)\
    ...
    #table2 = spark.sql('select main_table.track_id, main_table.score, tag_table.tags from main_table left join index_table on\
